...
import mx.com.factico.cide.fragments.PropuestasPageFragment;
...
public class PropuestasActivity extends ActionBarActivity {
	...
	private void setSupportActionBar() {
		...
		mToolbar.setTitle(getResources().getString(R.string.propuesta_name));
	...
	private void startFacebookLoginIntent() {
		Intent intent = new Intent(PropuestasActivity.this, FacebookLoginActivity.class);
	...
	private void setTabsToActioBar() {
		mTabHost = (TabHost) findViewById(R.id.propuestas_tabhost);
		mTabHost.setup();
		...
		mTabHost.setOnTabChangedListener(new OnTabChangeListener() {
			...
			public void onTabChanged(String tabId) {
				mViewPager.setCurrentItem(Integer.parseInt(tabId), false);
			}
		});
		
		mViewPager = (ViewPager) findViewById(R.id.propuestas_pager);
		...
		for (int i = 0; i < listCategories.length; i++) {
			String string = listCategories[i];
			mPagerAdapter.addFragment(PropuestasPageFragment.newInstance(i, Color.YELLOW));
		}
	...
	private static View createTabView(Context context, int index) {
		...
		//TextView tv = (TextView) view.findViewById(R.id.item_tabhost_tv_title);
		//tv.setText(text);
		return view;
	}
		...
		if (listItems != null && listItems.size() > 0) {
			...
			for (int count = 0; count < listItems.size(); count++) {
				...
				if (item != null) {
					 ...
					 * hasInCategory = true; i += 1;
					 * 
					 * if (i <= countToShow) { View view = createItemView(item);
					 * containerTestimonios.addView(view); } else { break; } }
					 */
				}
			}
